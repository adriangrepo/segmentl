...
from segmentl.distance.utils import compute_edts_batch
...
class BoundaryLoss(nn.Module):
    ...
    The surrogate is not used alone fortraining, but as a weighted sum with IoU loss (from directIoUoptimization).
    We found that the impact of the boundary component of the loss function should begradually increased during training,
    ...
    'forLBF1,IoUloss, it requires an additional procedure for mini grid-search:
    ...
    def forward(self, pred, gt):
        ...
        if self.debug:
            for j, k, l, m in zip(gt_b_ext, pred_b_ext, gt_b, pred_b):
                ...
                plt.savefig(f'images/BoundaryLoss_gt_b_ext.png')
                plt.imshow(k[0,:,:].detach().cpu().numpy())
                plt.savefig(f'images/BoundaryLoss_pred_b_ext.png')
                plt.imshow(l[0,:,:].detach().cpu().numpy())
                plt.savefig(f'images/BoundaryLoss_gt_b.png')
                plt.imshow(m[0,:,:].detach().cpu().numpy())
                plt.savefig(f'images/BoundaryLoss_pred_b.png')
