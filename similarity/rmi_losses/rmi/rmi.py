...
from segmentl.similarity.rmi_losses.rmi import rmi_utils
...
class RMILoss(nn.Module):
	...
	def forward(self, logits_4D, labels_3D):
		...
		#loss = self.forward_softmax_sigmoid(logits_4D, labels_3D)
		return loss

	def forward_sigmoid(self, logits_4D, labels_3D):
		"""
		Using sigmiod activation.
		Args:
		...
		print(f'valid_onehot_label_flat: {valid_onehot_label_flat.shape}')
		print(f'label_mask_flat: {label_mask_flat.shape}')
		print(f'label_mask_flat.unsqueeze(dim=1): {label_mask_flat.unsqueeze(dim=1).shape}')
